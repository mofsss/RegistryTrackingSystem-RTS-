			...
			if(strlen($_POST['password'])>=6)
			{
				...
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					if($dn['nb']==0 or $_POST['username']==$_SESSION['username'])
					{
						...
						if(mysqli_query($con,'UPDATE users set username="'.$username.'", password="'.$password.'", email="'.$email.'" where id="'.mysqli_real_escape_string($con,$_SESSION['userid']).'"'))
						{
...
<?php
	...
	if($form)
	{
		...
		else
		{
			...
			$dnn = mysqli_fetch_array(mysqli_query($con,'select username,password,email from users where username="'.$_SESSION['username'].'"'));
